    ...
    def __init__(self):
        ...
        with open("cgi-bin/elements.csv", encoding='utf-8') as fin:
            for line in fin:
                ...
                is_stable = not parameters[5].startswith('[')
                atomic_mass = re.search(regex, parameters[5]).group(0)[int(not is_stable):]
                if ',' in atomic_mass:
                    atomic_mass = float('.'.join(atomic_mass.split(sep=',')))
                else:
                    atomic_mass = int(atomic_mass)
                discovery_year = int(parameters[6])
                self.elements.append(Element(number, name, symbol, period, group, atomic_mass, is_stable, discovery_year))

    def get_undiscovered_elements(self, given_year):
        undiscovered_elements = []
        for element in self.elements:
            if not element.show(given_year):
                undiscovered_elements.append(element)
        return undiscovered_elements
